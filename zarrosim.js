...
function SetDlgItemTextEm(windowId,nIDDlgItem,lpString){ let control = document.querySelector('#win' + windowId + ' input.control' + nIDDlgItem); if (control != null) { control.value = UTF8ToString(lpString); return true; } control = document.querySelector('#win' + windowId + ' .control' + nIDDlgItem); if (control != null) { control.innerText = UTF8ToString(lpString); return true; } else { return false; } }
...
function __asyncjs__MessageBoxEm(windowId,lpText,lpCaption,uType,parentWindowId){ return Asyncify.handleAsync(async () => { const element = document.getElementById('messagebox'); const wall = document.getElementById('wall').cloneNode(true); const c = element.cloneNode(true); wall.id = 'wall' + windowId; c.id = 'win' + windowId; if (parentWindowId >= 0) { const parent = document.getElementById('win' + parentWindowId); if (parent != null) { const style = getComputedStyle(parent); c.style.left = (parseInt(style.left) + 40) + 'px'; c.style.top = (parseInt(style.top) + 40) + 'px'; } } wall.style.zIndex = '' + windowId; c.style.zIndex = '' + windowId; c.style.position = 'absolute'; c.querySelector('.title-bar-text').innerText = UTF8ToString(lpCaption); c.querySelector('.content').innerText = UTF8ToString(lpText); const destination = document.getElementById('screen'); destination.appendChild(wall); destination.appendChild(c); makeDraggable(c); const result = await waitListener(windowId); destination.removeChild(wall); destination.removeChild(c); return result; }); }
...
var asmLibraryArg = {
  ...
  "__asyncjs__DialogBoxWaitEvent": __asyncjs__DialogBoxWaitEvent,
  "__asyncjs__LoadStringEm": __asyncjs__LoadStringEm,
