...
function __asyncjs__DialogBoxEm(dialogId,dialog){ return Asyncify.handleAsync(async () => { const response = await fetch("resources/dialogs/includes/" + dialog + ".inc.html"); const html = await response.text(); const c = fromHTML(html); c.id = 'win' + dialogId; const destination = document.getElementById('screen'); destination.appendChild(c); addMainMenu(c); makeDraggable(c); }); }
function __asyncjs__DialogBoxWaitEvent(dialogId){ return Asyncify.handleAsync(async () => { return await waitListenerS(`#win${dialogId} button, #win${dialogId} input, #win${dialogId} .menu`, "click"); }); }
...
function __asyncjs__MessageBoxEm(dialogId,lpText,lpCaption,uType){ return Asyncify.handleAsync(async () => { const element = document.getElementById('messagebox'); const win = element.cloneNode(true); win.id = 'win' + dialogId; win.style.zIndex = '' + dialogId; win.style.position = 'absolute'; win.querySelector('.title-bar-text').innerText = UTF8ToString(lpCaption); win.querySelector('.content').innerText = UTF8ToString(lpText); const destination = document.getElementById('screen'); destination.appendChild(win); await waitListener(win.querySelector("button"), "click"); destination.removeChild(win); return 0; }); }
