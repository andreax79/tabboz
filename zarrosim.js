...
function __asyncjs__DialogBoxEm(windowId,dialog,parentWindowId){ return Asyncify.handleAsync(async () => { const response = await fetch("resources/dialogs/includes/" + dialog + ".inc.html"); const html = await response.text(); const c = createElementFromHTML(html); c.id = 'win' + windowId; if (parentWindowId >= 0) { const parent = document.getElementById('win' + parentWindowId); if (parent != null) { const style = getComputedStyle(parent); c.style.left = (parseInt(style.left) + 40) + 'px'; c.style.top = (parseInt(style.top) + 40) + 'px'; } } const destination = document.getElementById('screen'); destination.appendChild(c); addMainMenu(c); makeDraggable(c); }); }
function __asyncjs__DialogBoxWaitEvent(windowId){ return Asyncify.handleAsync(async () => { return await waitListenerS(`#win${windowId} button, #win${windowId} input, #win${windowId} .menu, #win${windowId} img`, "click"); }); }
...
function __asyncjs__MessageBoxEm(windowId,lpText,lpCaption,uType,parentWindowId){ return Asyncify.handleAsync(async () => { const element = document.getElementById('messagebox'); const c = element.cloneNode(true); c.id = 'win' + windowId; if (parentWindowId >= 0) { const parent = document.getElementById('win' + parentWindowId); if (parent != null) { const style = getComputedStyle(parent); c.style.left = (parseInt(style.left) + 40) + 'px'; c.style.top = (parseInt(style.top) + 40) + 'px'; } } c.id = 'win' + windowId; c.style.zIndex = '' + windowId; c.style.position = 'absolute'; c.querySelector('.title-bar-text').innerText = UTF8ToString(lpCaption); c.querySelector('.content').innerText = UTF8ToString(lpText); const destination = document.getElementById('screen'); destination.appendChild(c); makeDraggable(c); const result = await waitListenerS(`#win${windowId} button, #win${windowId} input, #win${windowId} .menu, #win${windowId} img`, "click"); destination.removeChild(c); return result; }); }
