...
function __asyncjs__MessageBoxEm(windowId,lpText,lpCaption,uType){ return Asyncify.handleAsync(async () => { const element = document.getElementById('messagebox'); const win = element.cloneNode(true); win.id = 'win' + windowId; win.style.zIndex = '' + windowId; win.style.position = 'absolute'; win.querySelector('.title-bar-text').innerText = UTF8ToString(lpCaption); win.querySelector('.content').innerText = UTF8ToString(lpText); const destination = document.getElementById('screen'); destination.appendChild(win); await waitListener(win.querySelector("button"), "click"); destination.removeChild(win); return 0; }); }
